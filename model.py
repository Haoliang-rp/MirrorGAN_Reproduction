...
class CA_NET(nn.Module):
    ...
    def reparametrize(self, mu, logvar):
        ...
        eps = Variable(eps)
        return eps.mul(std).add_(mu)    # shape (16, 300) sample from normal dist  -- mul--  (16, 300)  -- add -- mu (16, 100)
...
class INIT_STAGE_G(nn.Module):
    ...
    def forward(self, z_code, c_code):
        ...
        out_code = self.upsample1(out_code) # [1, 32 * 16, 4, 4] -> [1, 32 * 16, 8, 8](upsample) -> [1, 32 * 16, 8, 8](conv3X3) -> [1, 32 * 8, 8, 8](GLU)([:, :nc] is only effective for the second dimension)
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, word_embs, mask):
        ...
        # state size ngf/2 x 2in_size x 2in_size
        out_code = self.upsample(out_code)        # (16, 64, 64, 64) -> (16, 32, 128, 128)(second G net) -> (16, 32, 256, 256)(third G net)
        return out_code, att
